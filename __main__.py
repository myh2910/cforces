...
HELP = {
    "cat": {"usage": "cat <file>", "description": "Output file contents."},
    "cd": {"usage": "cd [directory]", "description": "Change the working directory."},
    "exec": {"usage": "exec <command>", "description": "Execute command on terminal."},
    "gcc": {
        "usage": "gcc [directory]",
        "description": "Compile the C++ file from the given project directory.",
    },
    "help": {"usage": "help [option]", "description": "Print help message."},
    "ls": {"usage": "ls [file]", "description": "List directory contents."},
    "code": {
        "usage": "code <directory>",
        "description": "Create a programming project according to the current mode.",
    },
    "mkdir": {"usage": "mkdir <directory>", "description": "Make new directory."},
    "mode": {
        "usage": "mode <mode>",
    ...
    "rm": {"usage": "rm <file>", "description": "Remove files or directories."},
    "run": {
        "usage": "run [directory]",
...
MODE = {
    "codeforces": Path(__file__, "../templates/codeforces.cpp").resolve().read_text(),
    "usaco": Path(__file__, "../templates/usaco.cpp").resolve().read_text(),
}
...
def scan_files(path):
    ...
    for file in os.scandir(path):
        ...
        if file.is_dir():
            ...
            filename = file.name
        print(
            "\033[34m%s─ %s%s\033[0m" % ("╰" if cnt == total else "├", color, filename)
        )
...
def open_file(path):
    ...
    if platform.system() == "Windows":
        ...
        os.system('xfce4-terminal -x nvim "%s"' % path)
    return signal.DONE
...
def coding_project(path):
    ...
    if os.path.exists(path):
        ...
        with open(cpp_file, "w", newline="\n") as f:
            if CONFIG["mode"] == "codeforces":
                f.write(MODE["codeforces"])
            elif CONFIG["mode"] == "usaco":
                f.write(MODE["usaco"] % (project, project, project))
    ...
    if platform.system() == "Windows":
        ...
        keyboard.press_and_release(
            "ctrl+pagedown,ctrl+alt+right,ctrl+pageup,ctrl+alt+right,ctrl+alt+right"
        )
...
def remove_file(path):
    ...
    try:
        proceed = input(
            "Directory '%s' is not empty; confirm to continue [y/N] \033[97m" % path
        ).strip()
        print("\033[0m", end="")
        if proceed in ("y", "Y"):
            shutil.rmtree(path)
